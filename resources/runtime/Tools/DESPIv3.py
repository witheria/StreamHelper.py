...
#
# The "use a different name" can be used, if the json uses another source folder name than its actually inside of
#
...
from PyQt5.QtWidgets import QMainWindow, QFileDialog, QGraphicsOpacityEffect
...
Icon = savestate.SOURCE_PATH + "/images/common/icon2.png"
Background = savestate.SOURCE_PATH + "/images/common/icon2.png"


class Despi(QMainWindow):
    def __init__(self, MainWindow):
        ...
        self.install.setGeometry(QtCore.QRect(8, 90, 111, 23))
        ...
        self.label_2.setGeometry(QtCore.QRect(130, 40, 130, 16))
        ...
        self.customName = QtWidgets.QCheckBox(self.centralwidget)
        self.customName.setText("Use a different source folder name")
        self.customName.setGeometry(QtCore.QRect(8, 60, 190, 23))
        self.customName.setObjectName("checkbox")

        self.delete = QtWidgets.QPushButton(self.centralwidget)
        self.delete.setGeometry(QtCore.QRect(122, 90, 111, 23))
        self.delete.setObjectName("pushButton_2")

        self.close = QtWidgets.QPushButton(self.centralwidget)
        self.close.setGeometry(QtCore.QRect(237, 90, 111, 23))
        ...
        self.install.clicked.connect(lambda: self.installPathsNoCopy())

        self.delete.clicked.connect(lambda: self.installPathsCopy())
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.delete.setText(_translate("MainWindow", "Copy Package"))
        self.close.setText(_translate("MainWindow", "Cancel"))
        self.finish.setText(_translate("MainWindow", ""))

    def installPathsCopy(self):
        print("\nSetting up copying of files...\n")
        _translate = QtCore.QCoreApplication.translate
        path = QFileDialog.getExistingDirectory(self.centralwidget, "Target folder",
                                                powerPathing["packagePath"])
        powerPathing["userPath"] = path

        file = QFileDialog.getOpenFileName(self.centralwidget, "Please choose the json", powerPathing["packagePath"])

        powerPathing["jsonfile"] = file[0]
        powerPathing["jsonpath"] = os.path.split(powerPathing["jsonfile"])[0]
        print("Got JSON-File: {}".format(powerPathing["jsonfile"]))

        if self.customName.isChecked():
            inputName = QtWidgets.QInputDialog()
            subdirlist, ok = inputName.getText(self.centralwidget, "Please give the old source folder",
                                               "Please give the old folder name:")
            if not ok or len(subdirlist) < 1:
                print("Please give a proper folder name!")
                return
        else:
        ...
        try:
            changeJSONCopyPackage(subdirlist)
            getOBSPath(0)
        except FileNotFoundError:
            pass

    def installPathsNoCopy(self):
        _translate = QtCore.QCoreApplication.translate
        ...
        if self.customName.isChecked():
            inputName = QtWidgets.QInputDialog()
            subdirlist, ok = inputName.getText(self.centralwidget, "Please give a source folder",
                                               "Please give the old folder name:")
            if not ok or len(subdirlist) < 1:
                print("Please give a proper folder name!")
                return
        ...
        try:
            changeJSONnoCopy(subdirlist)
            getOBSPath(0)
        except FileNotFoundError:
            pass


def changeJSONnoCopy(subdirlist):
    print(subdirlist)
    try:
        with open(powerPathing["jsonfile"], "r", encoding="utf-8") as subfile:
        ...
        for key in jsonPathNames:
            ...
            if key in i["settings"]:
                ...
                if subdirlist in save:
                    newName = powerPathing["jsonpath"] + save.split(subdirlist)[1]
                    print("This is the new path: ", newName)
                    i['settings'][key] = newName
                else:
                    print(f"{i} seems to not be part of the package!")
            if "playlist" in i["settings"] and i["id"] == "vlc_source":
                for setting in range(0, len(i["settings"]["playlist"])):
                    ...
                    if subdirlist in save["value"]:
                        newName = powerPathing["jsonpath"] + save["value"].split(subdirlist)[1]
    ...
    for i in subjsonobject["transitions"]:
        if subdirlist in i["settings"]["path"]:
            ...
            newName = powerPathing["jsonpath"] + i["settings"]["path"].split(subdirlist)[1]
            i["settings"]["path"] = newName
            print("New transition path is: ", newName)
        else:
            print("\n")

    packageFile = powerPathing["jsonfile"]
...
def changeJSONCopyPackage(subdirlist):
    try:
        ...
        for key in jsonPathNames:
            ...
            if "playlist" in i["settings"] and i["id"] == "vlc_source":
                for setting in range(0, len(i["settings"]["playlist"])):
                    ...
                    print("Found playlist: ", save["value"])
                    if subdirlist in save["value"]:
                        newName = powerPathing["userPath"] + "/" + powerPathing["name"] + \
                                  save["value"].split(subdirlist)[1]
                        print("This is the new path for the playlist: ", newName)
                        i["settings"]["playlist"][setting]["value"] = newName
            else:
                print("The path seems to not be part of the package!")
    ...
    json.dump(subjsonobject, subfile, sort_keys=True, indent=4)
    subfile.close()
    powerPathing["jsonfile"] = packageFile
